...
local function ensure_placeholder_ssl(home)
    ...
    local cmd = string.format(
        ...
        .. '-keyout %s -out %s -subj "/CN=APIOAK" 2>/dev/null',
        key_path, cert_path
    )
...
--- 主入口: 读取 apioak.yaml + 模板 → 渲染 → 写入 conf/nginx.conf
function _M.generate()
    local home = common.apioak_home

    -- 1. 读取 apioak.yaml
    local yaml_path = home .. "/conf/apioak.yaml"
    ...
    if not config then
        print("Generate nginx.conf    ...FAIL (invalid apioak.yaml)")
        os.exit(1)
    end

    -- 2. 加载模板 (从 Lua 模块)
    local template = require("apioak.cli.templates.nginx_conf")
