...
local pcall = pcall
local ipairs = ipairs
local core_config = require("apioak.core.config")
local core_log = require("apioak.core.log")
local core_string = require("apioak.core.utils.string")
local core_table = require("apioak.core.utils.table")
...
local function plugins_loading()
    ...
    for i = 1, #plugins do
        local plugin_path = core_string.format("apioak.plugin.%s.handler", plugins[i])
