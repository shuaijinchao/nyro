...
local function generate_backend_balancer(backend_data)
    ...
    local backend_balancer = {
        algorithm       = algorithm,
        read_timeout    = timeout_config.read or pdk.const.UPSTREAM_DEFAULT_TIMEOUT,
        write_timeout   = timeout_config.send or pdk.const.UPSTREAM_DEFAULT_TIMEOUT,
        connect_timeout = timeout_config.connect or pdk.const.UPSTREAM_DEFAULT_TIMEOUT,
    }
...
function _M.gogogo(oak_ctx)
    if not oak_ctx.config or not oak_ctx.config.service_router or not oak_ctx.config.service_router.router then
        pdk.log.error("[sys.balancer] oak_ctx.config.service_router.router is null!")
        return
    end
    ...
    if not upstream or not next(upstream) then
        pdk.log.error("[sys.balancer] upstream is null!")
        return
    end
    ...
    if upstream.id then
        ...
        if not backend_obj then
            pdk.log.error("[sys.balancer] backend not found: ", upstream.id)
            return
        end
        ...
        local address_port
        if backend_obj.algorithm == pdk.const.BALANCER_ROUNDROBIN then
            address_port = backend_obj.handler:find()
        elseif backend_obj.algorithm == pdk.const.BALANCER_CHASH then
        ...
        if not address_port then
            pdk.log.error("[sys.balancer] backend handler find null!")
            return
        end

        local parts = pdk.string.split(address_port, "|")
        if #parts ~= 2 then
            pdk.log.error("[sys.balancer] address:port format error: ", address_port)
            return
        end
        ...
        if not upstream.port then
            pdk.log.error("[sys.balancer] upstream port undefined")
            return
        end
        ...
        elseif upstream.address and is_ip_address(upstream.address) then
            ...
            pdk.log.error("[sys.balancer] upstream address not resolved: ", upstream.address)
            return
        end
    ...
    if not address or not port then
        pdk.log.error("[sys.balancer] address or port is null")
        return
    end
    ...
    if err2 then
        pdk.log.error("[sys.balancer] port check error: ", port, " ", err2)
        return
    end
    ...
    if not ok then
        pdk.log.error("[sys.balancer] set timeouts error: ", timeout_err)
        return
    end
    ...
    if not ok2 then
        pdk.log.error("[sys.balancer] set peer error: ", peer_err)
        return
    end
end
