/*
 * NYRO Router Engine Implementation
 * 
...
static int route_list_add(route_list_t *list, route_entry_t *entry) {
    if (list->count >= list->capacity) {
        ...
        if (!new_entries) return NYRO_ROUTER_ERR_NOMEM;
        list->entries = new_entries;
        list->capacity = new_cap;
    }
    list->entries[list->count++] = entry;
    return NYRO_ROUTER_OK;
}
...
static int match_param_path(const char *pattern, size_t pattern_len,
    ...
    size_t pi = 0, ri = 0;
    int param_count = 0;
    nyro_router_param_t params[16];  /* 最多 16 个参数 */
    ...
    if (param_count > 0 && result) {
        result->params = malloc(param_count * sizeof(nyro_router_param_t));
        if (result->params) {
            memcpy(result->params, params, param_count * sizeof(nyro_router_param_t));
...
nyro_router_t *nyro_router_new(void) {
    nyro_router_t *router = malloc(sizeof(nyro_router_t));
...
int nyro_router_add(nyro_router_t *router,
    ...
    if (!router || !path || path_len == 0) {
        return NYRO_ROUTER_ERR_INVALID;
    }
    ...
    route_entry_t *entry = route_entry_new(host, path, path_len, 
                                            methods, match_type, 
                                            priority, handler);
    if (!entry) return NYRO_ROUTER_ERR_NOMEM;
...
int nyro_router_build(nyro_router_t *router) {
    if (!router) return NYRO_ROUTER_ERR_INVALID;
...
int nyro_router_match(nyro_router_t *router,
    ...
    if (entry != raxNotFound) {
        if (entry->methods & method) {
            ...
            result->match_type = NYRO_ROUTER_MATCH_EXACT;
            return 1;
        }
    }
    ...
    while (raxPrev(&iter)) {
        ...
        if (iter.key_len <= path_len && 
            ...
            if (iter.key_len == path_len || path[iter.key_len] == '/') {
                ...
                if (entry->methods & method) {
                    result->handler = entry->handler;
                    result->match_type = NYRO_ROUTER_MATCH_PREFIX;
                    raxStop(&iter);
                    return 1;
                }
            }
            ...
            for (size_t i = 0; i < list->count; i++) {
                ...
                if (entry->methods & method) {
                    if (match_param_path(entry->path, entry->path_len,
                        ...
                        result->handler = entry->handler;
                        result->match_type = NYRO_ROUTER_MATCH_PARAM;
                        raxStop(&iter);
                        return 1;
                    }
                }
