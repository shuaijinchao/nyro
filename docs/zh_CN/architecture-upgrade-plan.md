# APIOAK 架构升级需求文档

> 版本: v2.0  
> 日期: 2026-01-30  
> 状态: **开发中** (DB Less 模式开发中)

---

## 目录

1. [概述](#概述)
2. [实施进度](#实施进度)
3. [资源模型设计](#资源模型设计)
4. [需求一：重构路由引擎](#需求一重构路由引擎)
5. [需求二：移除 Consul 依赖](#需求二移除-consul-依赖)
6. [需求三：AI Proxy 能力](#需求三ai-proxy-能力)
7. [技术路线图](#技术路线图)
8. [配置文件设计](#配置文件设计)

---

## 概述

本次架构升级旨在提升 APIOAK 的性能、简化部署复杂度、并增加对 AI 场景的支持。主要包含三大核心改动：

| 序号 | 需求 | 目标 |
|------|------|------|
| 1 | 重构路由引擎 | 使用 Radix Tree 实现高性能路由，支持多种匹配模式 |
| 2 | 移除 Consul 依赖 | 实现 DB Less / Hybrid 模式，降低部署复杂度 |
| 3 | AI Proxy 能力 | 支持 LLM 协议标准化、多 Key 管理、故障转移 |

### 开发阶段规划

| 阶段 | 内容 | 状态 |
|------|------|------|
| **Phase 1: DB Less** | 声明式配置、新资源模型、路由引擎 | 🚧 开发中 |
| **Phase 2: Hybrid** | CP/DP 分离、MongoDB 集成 | ⏳ 待开发 |
| **Phase 3: AI Proxy** | LLM 协议标准化、多 Key 管理 | ⏳ 待开发 |

---

## 实施进度

### Phase 1: DB Less 模式

| 步骤 | 状态 | 完成日期 |
|------|------|----------|
| Step 1: FFI 路由引擎 (Rax + Khash) | ✅ 已完成 | 2026-01-30 |
| Step 2: Store 抽象层 | ✅ 已完成 | 2026-01-30 |
| Step 3: 系统集成 (Router + Balancer) | ✅ 已完成 | 2026-01-30 |
| Step 4: Consul 移除与清理 | ✅ 已完成 | 2026-01-30 |
| Step 5: 正则匹配支持 | ✅ 已完成 | 2026-01-30 |
| Step 6: 新资源模型实现 | 🚧 开发中 | - |

### 已完成工作详情

#### 1. FFI 路由引擎 (Rax + Khash)

选择了 **方案 B: 自行封装 Rax FFI**，实现了基于 C 的高性能路由引擎。

**新增文件:**

| 文件 | 说明 |
|------|------|
| `deps/apioak/apioak_router.h` | C 头文件，定义路由引擎接口 |
| `deps/apioak/apioak_router.c` | C 实现，基于 Rax (Radix Tree) + Khash |
| `deps/apioak/Makefile` | C 库编译脚本 |
| `deps/rax/rax.h` | Rax 库头文件 |
| `deps/rax/rax.c` | Rax 库实现 |
| `apioak/sys/router/ffi.lua` | LuaJIT FFI 绑定定义 |
| `apioak/sys/router/matcher.lua` | Lua 封装层，提供面向对象 API |
| `apioak/sys/router/init.lua` | 路由模块入口 |

**支持的匹配类型:**

| 类型 | 示例 | 自动推导规则 | 状态 |
|------|------|-------------|------|
| 精确匹配 | `/api/v1/users` | 默认 | ✅ 已实现 |
| 参数匹配 | `/api/v1/users/{id}` | 包含 `{param}` | ✅ 已实现 |
| 前缀匹配 | `/api/v1/articles/*` | 以 `/*` 结尾 | ✅ 已实现 |
| 正则匹配 | `~^/api/v[0-9]+/.*` | 以 `~` 开头 | ✅ 已实现 |

#### 2. Store 抽象层 (DB Less 模式)

实现了存储抽象层，支持从 YAML 文件声明式加载配置。

**新增文件:**

| 文件 | 说明 |
|------|------|
| `apioak/store/init.lua` | Store 抽象层入口 |
| `apioak/store/adapter/yaml.lua` | YAML 适配器 (DB Less 模式) |
| `conf/config.yaml` | 声明式配置文件 |

#### 3. 系统集成

将新的路由引擎和 Store 层集成到系统启动流程。

**修改文件:**

| 文件 | 改动内容 |
|------|---------|
| `apioak/sys/router.lua` | 完全重写，使用 FFI 路由引擎 + Store |
| `apioak/sys/balancer.lua` | 重写，使用 Store 获取 upstream 数据 |
| `apioak/sys/dao.lua` | 重写，使用 Store 初始化 |
| `apioak/sys/admin.lua` | 重写，standalone 模式下禁用 Admin API |
| `apioak/cmd/env.lua` | 移除 Consul 检查，添加 Store 配置验证 |

#### 4. Consul 移除

彻底移除 Consul 依赖，保留向后兼容的存根模块。

---

## 资源模型设计

### 资源层次结构

```
┌─────────────────────────────────────────────────────────────┐
│                        请求入口                              │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│  routes (路由)                                               │
│  ├── paths, methods, hosts, headers (匹配条件)              │
│  ├── priority, strip_path, preserve_host                    │
│  ├── plugins (路由级插件)                                    │
│  └── service ──────────────────────┐ (必须引用)             │
└─────────────────────────────────────│────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────┐
│  services (服务) - upstream 的逻辑抽象                       │
│  ├── protocol, path                                         │
│  ├── plugins (服务级插件)                                    │
│  └── backend ──────┐  或  url (直接代理)                     │
└────────────────────│────────────────────────────────────────┘
                     │
                     ▼
┌─────────────────────────────────────────────────────────────┐
│  backends (后端) - 对应 Nginx upstream                       │
│  ├── endpoints (端点列表)                                    │
│  ├── algorithm (负载均衡算法)                                │
│  ├── timeout, retries                                       │
│  └── health_check                                           │
└─────────────────────────────────────────────────────────────┘

┌─────────────────────┐    ┌─────────────────────┐
│  plugins (全局)      │    │  consumers          │
│  对所有请求生效       │    │  认证主体 + 凭证     │
└─────────────────────┘    └─────────────────────┘

┌─────────────────────┐
│  certificates       │
│  SSL/TLS 证书       │
└─────────────────────┘
```

### 核心规则

| 规则 | 说明 |
|------|------|
| route 必须引用 service | route 不直接与 backend 关联 |
| service 是 upstream 的逻辑抽象 | 可引用 backend 或直接使用 url |
| 配置存在即加载 | 无 enabled 字段，配置即生效 |
| 插件使用 name 指定 | 全局和内联插件统一使用 name 字段 |

### 资源定义

#### 1. plugins (全局插件)

对所有请求生效的插件配置。

```yaml
plugins:
  - name: cors
    config:
      allow_origins: "*"
      allow_methods: ["GET", "POST", "PUT", "DELETE"]
  
  - name: request-id
    config:
      header_name: X-Request-ID
```

#### 2. backends (后端)

对应 Nginx upstream，定义后端服务集群。

```yaml
backends:
  - name: user-backend
    algorithm: roundrobin              # roundrobin | chash | least_conn
    endpoints:                         # 端点列表
      - address: 192.168.1.10:9001
        weight: 100
      - address: 192.168.1.11:9002
        weight: 100
    timeout:
      connect: 5000                    # 连接超时 (ms)
      send: 60000                      # 发送超时 (ms)
      read: 60000                      # 读取超时 (ms)
    retries: 3                         # 重试次数
    health_check:
      type: http                       # http | tcp | https
      path: /health
      interval: 5                      # 检查间隔 (s)
      timeout: 2                       # 检查超时 (s)
      healthy:
        successes: 2
        http_statuses: [200, 302]
      unhealthy:
        failures: 3
        http_statuses: [500, 502, 503]
```

#### 3. services (服务)

upstream 的逻辑抽象，可引用 backend 或直接代理 URL。

```yaml
services:
  # 方式1: 引用 backend
  - name: user-service
    backend: user-backend
    protocol: http                     # http | https | grpc | grpcs
    path: /user-center                 # 上游路径前缀 (可选)
    plugins:
      - name: request-id

  # 方式2: 直接代理 URL
  - name: external-api
    url: https://api.external.com
    protocol: https
```

#### 4. routes (路由)

请求匹配规则，必须引用一个 service。

```yaml
routes:
  - name: user-login
    service: user-service              # 必须引用 service
    hosts:
      - api.example.com
    paths:
      - /api/v1/login
    methods: [POST]
    priority: 100                      # 路由优先级
    strip_path: false                  # 是否剥离匹配路径
    preserve_host: true                # 是否保留原始 Host
    plugins:
      - name: jwt-auth
      - name: rate-limiting
        config:
          rate: 100
          burst: 50
```

**路由匹配类型:**

| 类型 | 路径示例 | 自动推导规则 |
|------|---------|-------------|
| 精确匹配 | `/api/v1/users` | 默认 |
| 前缀匹配 | `/api/v1/articles/*` | 以 `/*` 结尾 |
| 参数匹配 | `/api/v1/users/{id}` | 包含 `{param}` |
| 正则匹配 | `~^/api/v[0-9]+/.*` | 以 `~` 开头 |

#### 5. consumers (消费者)

API 消费者，用于认证和限流。

```yaml
consumers:
  - name: mobile-app
    credentials:
      key-auth:
        key: mobile-app-api-key
      jwt-auth:
        key: mobile-app
        secret: jwt-secret
    plugins:
      - name: rate-limiting
        config:
          rate: 1000
          burst: 100
```

#### 6. certificates (证书)

SSL/TLS 证书配置。

```yaml
certificates:
  - name: example-cert
    snis:
      - "*.example.com"
      - "example.com"
    cert: |
      -----BEGIN CERTIFICATE-----
      ...
      -----END CERTIFICATE-----
    key: |
      -----BEGIN PRIVATE KEY-----
      ...
      -----END PRIVATE KEY-----
```

### 最小配置单元

#### 场景 1: 代理到内部后端

```yaml
backends:
  - name: api-backend
    endpoints:
      - address: 127.0.0.1:8080

services:
  - name: api-service
    backend: api-backend

routes:
  - name: api
    service: api-service
    paths:
      - /api/*
```

#### 场景 2: 代理到外部 URL

```yaml
services:
  - name: external-service
    url: https://api.external.com

routes:
  - name: external
    service: external-service
    paths:
      - /external/*
```

#### 场景 3: 带认证的 API

```yaml
backends:
  - name: api-backend
    endpoints:
      - address: 127.0.0.1:8080

services:
  - name: api-service
    backend: api-backend

routes:
  - name: api
    service: api-service
    paths:
      - /api/*
    plugins:
      - name: key-auth

consumers:
  - name: my-app
    credentials:
      key-auth:
        key: my-api-key
```

---

## 需求一：重构路由引擎

### 背景

当前使用的 `lua-resty-oakrouting` 是纯 Lua 实现，在大量路由场景下性能有限。需要升级为基于 Radix Tree 的高性能路由引擎。

### 目标

基于 **Rax (Radix Tree)** 实现高性能路由引擎，支持四种匹配模式。

### 技术选型

#### 方案 B: 自行封装 Rax FFI ✅ **已选择**

- **优点**: 轻量化，可针对 APIOAK 配置结构优化
- **状态**: **已实现** - 基于 Rax + Khash 实现高性能路由引擎

### 实现架构

```
┌─────────────────────────────────────────────────────────────┐
│                    Lua Layer (LuaJIT)                       │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  apioak/sys/router/init.lua     (模块入口)          │   │
│  ├─────────────────────────────────────────────────────┤   │
│  │  apioak/sys/router/matcher.lua  (面向对象封装)      │   │
│  ├─────────────────────────────────────────────────────┤   │
│  │  apioak/sys/router/ffi.lua      (FFI 定义)          │   │
│  └─────────────────────────────────────────────────────┘   │
│                            │                                │
│                       FFI 调用                              │
│                            ▼                                │
├─────────────────────────────────────────────────────────────┤
│                     C Layer (libapioak_router)              │
│  ┌───────────────────┐    ┌───────────────────┐            │
│  │   Rax (Radix Tree) │    │  Khash (Hash Table)│            │
│  │   - 路径索引       │    │  - Host 索引      │            │
│  │   - 前缀匹配       │    │  - 快速查找       │            │
│  └───────────────────┘    └───────────────────┘            │
└─────────────────────────────────────────────────────────────┘
```

### 优先级仲裁规则

当多条路由可能匹配同一请求时，按以下顺序决定：

1. **精确匹配** (优先级最高)
2. **参数匹配**
3. **前缀匹配** (最长前缀优先)
4. **正则匹配** (按 priority 排序)

---

## 需求二：移除 Consul 依赖

### 背景

移除 Consul 依赖，转向 **DB Less (声明式)** 和 **Hybrid (控制面/数据面分离)** 模式，是云原生网关的主流演进方向。

### 开发策略

| 阶段 | 模式 | 状态 |
|------|------|------|
| Phase 1 | DB Less (Standalone) | 🚧 开发中 |
| Phase 2 | Hybrid (CP/DP + MongoDB) | ⏳ 待 Phase 1 稳定后开发 |

### 模式一：DB Less (Standalone) - 当前阶段

#### 架构图

```
┌─────────────────────────────────────────────────┐
│                   APIOAK Node                   │
│  ┌───────────┐    ┌──────────────────────────┐ │
│  │config.yaml│───▶│  ngx.shared.DICT (内存)  │ │
│  └───────────┘    └──────────────────────────┘ │
│        │                      │                 │
│   HUP Signal ────────────▶ 热加载              │
└─────────────────────────────────────────────────┘
```

#### 核心逻辑

1. **配置载体**: 声明式 `config.yaml` 配置文件
2. **启动加载**: 在 `init_by_lua` 阶段解析 YAML → 校验 → 写入内存
3. **热更新**: 发送 HUP 信号触发重新加载

#### 适用场景

- K8s Ingress / Gateway API
- Ansible/SaltStack 管理
- GitOps 工作流

### 模式二：Hybrid (CP/DP 分离) - 待开发

> ⏳ 待 DB Less 模式稳定后开发

#### 架构图

```
┌─────────────────┐        ┌─────────────────────────────┐
│  Control Plane  │        │        Data Plane           │
│  ┌───────────┐  │        │  ┌───────────────────────┐  │
│  │ Admin API │  │        │  │   ngx.shared.DICT     │  │
│  └─────┬─────┘  │        │  └───────────▲───────────┘  │
│        │        │        │              │              │
│  ┌─────▼─────┐  │◀───────│──────HTTP Long Polling──────│
│  │  MongoDB  │  │        │                             │
│  └───────────┘  │        │  ┌───────────────────────┐  │
│                 │        │  │  Local Config Dump    │  │
└─────────────────┘        └──┴───────────────────────┴──┘
```

---

## 需求三：AI Proxy 能力

> ⏳ 待 DB Less 模式稳定后开发

### 背景

AI Proxy 是目前 API 网关最火热的场景，本质是七层流量治理在 LLM 场景下的特化。

### 目标

提供标准化的 AI 接口与治理能力，让用户可以用任意 OpenAI 兼容客户端无缝连接各种 LLM Provider。

### Phase 1: 协议标准化 (The Unifier)

| 功能 | 描述 |
|------|------|
| 对外接口 | 统一暴露 OpenAI 兼容接口 (`/v1/chat/completions`) |
| 请求转换 | 将请求适配为各厂商格式 (Anthropic, Google, DeepSeek 等) |
| 流式处理 | 统一 SSE (Server-Sent Events) 响应格式 |

### Phase 2: 算力调度与高可用

| 功能 | 描述 |
|------|------|
| 多 Key 轮询 | 配置多个 API Key，支持 Round-Robin 或加权轮询 |
| 智能重试 | 针对 429 / 503 自动重试 |
| 故障转移 | Provider 级别 Fallback |

---

## 技术路线图

### 执行顺序

```
┌────────────────────────────────────────────────────────────────┐
│  Phase 1: DB Less 模式                            🚧 开发中    │
│  ├── ✅ FFI 路由引擎 (Rax + Khash)                             │
│  ├── ✅ Store 抽象层 (YAML 适配器)                             │
│  ├── ✅ 系统集成 (Router + Balancer)                          │
│  ├── ✅ Consul 移除与清理                                      │
│  ├── ✅ 四种路由匹配模式                                       │
│  └── 🚧 新资源模型实现                                         │
│       ├── ⏳ backends 资源支持                                 │
│       ├── ⏳ services 资源支持                                 │
│       ├── ⏳ routes 资源支持                                   │
│       ├── ⏳ consumers 资源支持                                │
│       ├── ⏳ certificates 资源支持                             │
│       └── ⏳ plugins 资源支持                                  │
├────────────────────────────────────────────────────────────────┤
│  Phase 2: Hybrid 模式                             ⏳ 待开发    │
│  ├── ⏳ 集成 lua-resty-mongol                                  │
│  ├── ⏳ 实现 CP Admin API (CRUD → MongoDB)                     │
│  ├── ⏳ 实现 DP HTTP Long Polling 同步                         │
│  └── ⏳ 本地 Config Dump 容灾                                  │
├────────────────────────────────────────────────────────────────┤
│  Phase 3: AI Proxy 插件                           ⏳ 待开发    │
│  ├── ⏳ 协议标准化 + SSE 流式处理                              │
│  ├── ⏳ 多 Key 管理 + 故障转移                                 │
│  └── ⏳ Token 估算 + 缓存 (可选)                               │
└────────────────────────────────────────────────────────────────┘
```

### 依赖关系

```
Phase 1 (DB Less) 🚧 ──▶ Phase 2 (Hybrid) ⏳
                   │
                   └──▶ Phase 3 (AI Proxy) ⏳
```

---

## 配置文件设计

### apioak.yaml (系统配置)

```yaml
# APIOAK 系统配置文件

# 运行模式: standalone (DB Less) | hybrid (CP/DP 分离)
mode: standalone

# ============================================================
# Standalone 模式配置 (DB Less)
# ============================================================
standalone:
  config_file: conf/config.yaml
  reload_method: signal

# ============================================================
# Hybrid 模式配置 (待开发)
# ============================================================
# hybrid:
#   role: dp
#   control_plane:
#     listen:
#       host: 0.0.0.0
#       port: 9080
#     mongodb:
#       uri: mongodb://localhost:27017
#       database: apioak
#   data_plane:
#     control_plane_endpoints:
#       - http://cp-1.apioak.local:9080

# ============================================================
# 插件配置
# ============================================================
plugins:
  - cors
  - key-auth
  - jwt-auth
  - basic-auth
  - limit-req
  - limit-conn
  - limit-count
  - ip-restriction
  - request-id

# ============================================================
# 日志配置
# ============================================================
logging:
  level: info
  path: logs/

# ============================================================
# 性能配置
# ============================================================
performance:
  worker_processes: auto
  shared_dict_size: 64m
```

### config.yaml (声明式配置)

```yaml
#
# APIOAK 声明式配置文件 (DB Less 模式)
#

version: "1.0"

# ============================================================
# 全局插件
# ============================================================
plugins:
  - name: cors
    config:
      allow_origins: "*"
      allow_methods: ["GET", "POST", "PUT", "DELETE", "OPTIONS"]

  - name: request-id
    config:
      header_name: X-Request-ID
      include_in_response: true

# ============================================================
# 后端定义
# ============================================================
backends:
  - name: user-backend
    algorithm: roundrobin
    endpoints:
      - address: 192.168.1.10:9001
        weight: 100
      - address: 192.168.1.11:9002
        weight: 100
    timeout:
      connect: 5000
      send: 60000
      read: 60000
    retries: 3
    health_check:
      type: http
      path: /health
      interval: 5
      timeout: 2
      healthy:
        successes: 2
        http_statuses: [200, 302]
      unhealthy:
        failures: 3
        http_statuses: [500, 502, 503]

# ============================================================
# 服务定义
# ============================================================
services:
  - name: user-service
    backend: user-backend
    protocol: http
    path: /user-center
    plugins:
      - name: request-id

  - name: external-api
    url: https://api.external.com
    protocol: https

# ============================================================
# 路由定义
# ============================================================
routes:
  # 精确匹配
  - name: user-login
    service: user-service
    hosts:
      - api.example.com
    paths:
      - /api/v1/login
    methods: [POST]
    priority: 100

  # 参数匹配
  - name: user-profile
    service: user-service
    paths:
      - /api/v1/users/{id}
    methods: [GET, PUT, DELETE]
    priority: 90
    plugins:
      - name: jwt-auth

  # 前缀匹配
  - name: order-api
    service: user-service
    paths:
      - /api/v1/orders/*
    methods: [GET, POST, PUT, DELETE]
    priority: 80

  # 正则匹配
  - name: legacy-api
    service: external-api
    paths:
      - ~^/legacy/v[0-9]+/.*
    methods: [GET]
    priority: 10

# ============================================================
# 消费者定义
# ============================================================
consumers:
  - name: mobile-app
    credentials:
      key-auth:
        key: mobile-app-api-key
      jwt-auth:
        key: mobile-app
        secret: jwt-secret
    plugins:
      - name: rate-limiting
        config:
          rate: 1000
          burst: 100

# ============================================================
# SSL 证书定义
# ============================================================
certificates:
  - name: example-cert
    snis:
      - "*.example.com"
    cert: |
      -----BEGIN CERTIFICATE-----
      (certificate content)
      -----END CERTIFICATE-----
    key: |
      -----BEGIN PRIVATE KEY-----
      (private key content)
      -----END PRIVATE KEY-----
```

---

## 附录

### 参考项目

- [Apache APISIX](https://github.com/apache/apisix) - 路由引擎参考
- [Kong](https://github.com/Kong/kong) - Hybrid 模式参考
- [Envoy](https://github.com/envoyproxy/envoy) - 资源模型参考

### 相关 RFC

- RFC 8259: JSON 数据交换格式
- RFC 7540: HTTP/2
- RFC 8895: Server-Sent Events

---

*文档结束*
