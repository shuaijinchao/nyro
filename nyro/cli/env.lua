...
local function get_config()
    local res, err = io.open(common.nyro_home .. "/conf/nyro.yaml", "r")
    if not res then
...
local function validate_store()
    ...
    if mode == "standalone" then
        ...
        -- 检查配置文件是否存在
        local config_file_path = common.nyro_home .. "/" .. config_file
...
local function validate_plugin()
    ...
    for i = 1, #plugins do
        ...
        local file_path = common.nyro_home .. "/nyro/plugin/" .. plugins[i] .. "/handler.lua"
