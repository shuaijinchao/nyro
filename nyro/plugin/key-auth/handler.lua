...
local plugin_common = require("nyro.plugin.plugin_common")
...
function _M.http_access(oak_ctx, plugin_config)
    ...
    local header_key = matched.header["NYRO-KEY-AUTH"]

    if not header_key then
        pdk.response.exit(
                401, { message = "[key-auth] Authorization FAIL, header \"NYRO-KEY-AUTH\" is required" })
    end
